...
CameraArea::CameraArea(int left, int top, int right, int bottom, int weight) :
    m_left(left),
    m_top(top),
    m_right(right),
    m_bottom(bottom),
...
CameraArea::CameraArea(const char* str) {
    char* end;
...
int CameraArea::getX(int width) {
    return (((m_left + m_right) / 2) + 1000) * width / 2000;
}

int CameraArea::getY(int height) {
    return (((m_top + m_bottom) / 2) + 1000) * height / 2000;
}

bool CameraArea::isDummy() {
    return m_left == 0 && m_top == 0 && m_right == 0 && m_bottom == 0;
}

String8 CameraArea::toString8() {
